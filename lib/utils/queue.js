 ...
 * 动态并发数（可被配置覆盖）
 */
let dynamicMaxConcurrent = MAX_CONCURRENT;
/**
 * 添加任务到队列
 */
...
const processQueue = async () => {
    ...
    if (queue.length === 0 || processingCount >= dynamicMaxConcurrent) {
        return;
    }
    // 获取任务并增加处理计数
...
const processTask = async (taskData) => {
    ...
    resolve, // 成功回调
    config, // 渲染配置
    ...otherTaskData // 其他任务数据
     } = taskData;
    try {
        const manager = await getBrowserPoolManager();
        // 如果有配置，更新动态并发数
        if (config?.maxConcurrent !== undefined) {
            dynamicMaxConcurrent = config.maxConcurrent;
            console.log(`[jsxp] 动态并发数更新为: ${dynamicMaxConcurrent}`);
        }
        // 使用集群管理器执行任务，传递完整的任务数据和配置
...
export { MAX_CONCURRENT, addToQueue, getProcessing, processQueue, queue };
