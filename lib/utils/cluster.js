...
class BrowserPoolManager {
  constructor() {
    ...
    this.maxPagesPerBrowser = 10; // 页面池大小限制，避免过度积累
    this.pageCleanupTimer = null; // 页面清理定时器
    this.pageIdleTimeout = 2 * 60 * 1000; // 页面空闲超时2分钟（单位：毫秒）
  ...
  async init() {
    try {
      this.browserPool = new BrowserPool({
        browserPlugins: [
          new PlaywrightPlugin(chromium, this.playwrightOptions),
        ],
        maxOpenPagesPerBrowser: 5, // 每个浏览器的页面数上限
        retireBrowserAfterPageCount: 200, // 浏览器处理200个页面后自动退休
        closeInactiveBrowserAfterSecs: 5 * 60, // 闲置浏览器5分钟后自动关闭
  ...
  _startPerformanceMonitoring() {
    ...
    console.log("[jsxp] 页面清理已启动");
  }

  /**
   * 从页面池获取可复用页面
   * @returns {Promise<Page>} 可复用的页面
   */
  async _getPageFromPool() {
    ...
    if (
      ...
      console.log(
        `[jsxp] 复用页面，当前池大小: ${this.pagePool.get(defaultKey).length}`
      );
      return page;
    }
    // 没有空闲页面，创建新页面
   ...
   * @param {string} priority 任务优先级（high/normal/low）
   */
  async executeTask(taskData, priority = "normal") {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.activeTasks.size >= this.maxConcurrent) {
        ...
        this.taskQueue.push({ taskData, priority, resolve, reject });
        return;
      }
      // 立即添加到活跃任务集合，防止并发竞争
  ...
  async _executeTaskInternal(taskData) {
    ...
    try {
      ...
      if (error.message.includes("超时") || error.message.includes("Timeout")) {
        return this._fallbackScreenshot(taskData);
      }
      throw error;
    }
  }
   ...
   * 降级截图方案
   */
  async _fallbackScreenshot(taskData) {
    console.warn(`[jsxp] 任务 ${taskData.taskId} 使用降级模式`);
    let page = null;
    try {
      // 从页面池获取页面
      page = await this._getPageFromPool();
      ...
      const result = await this.executeScreenshot(page, {
        ...taskData,
        PupOptions: {
          ...taskData.PupOptions,
          goto: fallbackOptions,
        },
      });
      // 释放页面到池中
      await this._releasePageToPool(page);
      ...
      console.error(`[jsxp] 降级模式也失败:`, fallbackError.message);
      throw new Error("截图服务暂时不可用，请稍后重试");
    }
  }
